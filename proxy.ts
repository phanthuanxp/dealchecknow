import NextAuth from "next-auth";
import { NextResponse } from "next/server";

import authConfig from "@/lib/auth.config";
import {
  getClientIpFromHeaders,
  getCmsAdminRedirectUrl,
  getCmsLoginRateLimitMaxAttempts,
  getCmsLoginRateLimitWindowSeconds,
  isAdminPath,
  isCmsAdminIpAllowed,
  isCmsLoginRateLimitEnabled,
  isCmsRequestHost,
  isLoginRateLimitPath,
  shouldApplyCmsNoindex,
  shouldApplyCmsSecurityHeaders
} from "@/lib/cms-domain";

const { auth } = NextAuth(authConfig);

type LoginRateEntry = {
  count: number;
  resetAt: number;
};

const loginRateStore = new Map<string, LoginRateEntry>();

function checkLoginRateLimit(request: { nextUrl: { pathname: string }; method: string; headers: Headers }) {
  if (!isCmsLoginRateLimitEnabled()) {
    return null;
  }

  if (request.method.toUpperCase() !== "POST") {
    return null;
  }

  if (!isLoginRateLimitPath(request.nextUrl.pathname)) {
    return null;
  }

  const ip = getClientIpFromHeaders(request.headers);
  if (!ip) {
    return null;
  }

  const now = Date.now();
  const windowMs = getCmsLoginRateLimitWindowSeconds() * 1000;
  const maxAttempts = getCmsLoginRateLimitMaxAttempts();
  const key = `${ip}:${request.nextUrl.pathname}`;

  const existing = loginRateStore.get(key);
  if (!existing || existing.resetAt <= now) {
    loginRateStore.set(key, { count: 1, resetAt: now + windowMs });
    return null;
  }

  if (existing.count >= maxAttempts) {
    return Math.max(1, Math.ceil((existing.resetAt - now) / 1000));
  }

  existing.count += 1;
  loginRateStore.set(key, existing);
  return null;
}

function applyCmsHeaders(response: NextResponse, pathname: string) {
  if (shouldApplyCmsNoindex()) {
    response.headers.set("X-Robots-Tag", "noindex, nofollow, noarchive, nosnippet, noimageindex");
  }

  if (!shouldApplyCmsSecurityHeaders()) {
    return;
  }

  if (isAdminPath(pathname) || isLoginRateLimitPath(pathname)) {
    response.headers.set("X-Frame-Options", "DENY");
    response.headers.set("X-Content-Type-Options", "nosniff");
    response.headers.set("Referrer-Policy", "same-origin");
    response.headers.set("Permissions-Policy", "camera=(), microphone=(), geolocation=()");
    response.headers.set("Cache-Control", "no-store");
  }
}

export default auth((request) => {
  const cmsRedirectUrl = getCmsAdminRedirectUrl(request);
  if (cmsRedirectUrl) {
    return NextResponse.redirect(cmsRedirectUrl, 307);
  }

  if (!isCmsRequestHost(request)) {
    return;
  }

  if (!isCmsAdminIpAllowed(request)) {
    return NextResponse.json(
      {
        message: "Access denied from this IP address."
      },
      { status: 403 }
    );
  }

  const retryAfterSeconds = checkLoginRateLimit(request);
  if (retryAfterSeconds !== null) {
    return NextResponse.json(
      {
        message: "Too many login attempts. Please try again later."
      },
      {
        status: 429,
        headers: {
          "Retry-After": String(retryAfterSeconds)
        }
      }
    );
  }

  const response = NextResponse.next();
  applyCmsHeaders(response, request.nextUrl.pathname);
  return response;
});

export const config = {
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico|css|js|map|woff|woff2|ttf|eot)$).*)"
  ]
};
